...
理论上支持所有输入法，系统自带的微软拼音，微软五笔，搜狗输入法（请务必安装搜狗最新版），百度输入法，QQ输入法 …… 包括我测试的日文、韩文、西班牙语输入法都可用。

中文美式键盘：应当移除并改为「英语美式键盘」以避免英语键盘切换到中文状态。Win10/Win11 可在 ImTip 托盘菜单中禁用启用一次「英语键盘」就可以。Win7 请在控制面板添加「英语美式键盘」并设为默认，然后就可以移除「简体中文美式键盘」了。小鹤输入法：英文模式切换全角小鹤会返回错误状态码，按 shift 切换一次可修复正常。手心输入法：勾选「怪异模式」可用。
